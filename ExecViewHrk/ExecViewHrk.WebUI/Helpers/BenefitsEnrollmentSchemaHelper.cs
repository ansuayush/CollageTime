...
{
    public static class BenefitsEnrollmentSchemaHelper
    {
        public const string StorageFolderName = "BenefitsEnrollment";
...
");
        ...
        {
            if (!db.BenWaitingPeriods.Any())
            {
                db.BenWaitingPeriods.Add(new BenWaitingPeriod { Name = "Immediate", Days = 0, CalculationType = "Days", Description = "Eligible immediately", IsActive = true });
                db.BenWaitingPeriods.Add(new BenWaitingPeriod { Name = "30 Days", Days = 30, CalculationType = "Days", Description = "30 days after hire", IsActive = true });
                db.BenWaitingPeriods.Add(new BenWaitingPeriod { Name = "60 Days", Days = 60, CalculationType = "Days", Description = "60 days after hire", IsActive = true });
                db.BenWaitingPeriods.Add(new BenWaitingPeriod { Name = "90 Days", Days = 90, CalculationType = "Days", Description = "90 days after hire", IsActive = true });
                db.BenWaitingPeriods.Add(new BenWaitingPeriod { Name = "First Day Next Month", Days = 0, CalculationType = "FirstDayNextMonth", Description = "First day of month after hire", IsActive = true });
            }
            if (!db.BenCategories.Any())
            {
                var now = DateTime.Now;
                string[] cats = { "Medical", "Dental", "Vision", "Life Insurance", "Disability", "Critical Illness", "Accident Insurance", "HSA/FSA" };
                for (int i = 0; i < cats.Length; i++)
                {
                    db.BenCategories.Add(new BenCategory
                    {
                        CategoryName = cats[i],
                        Description = cats[i],
                        IsActive = true,
        ...
        {
            return "BEN-" + DateTime.Now.ToString("yyyyMMddHHmmss") + "-" + Guid.NewGuid().ToString("N").Substring(0, 6).ToUpperInvariant();
        }
    }
}
